...
;(function (window, document, $, undefined) {
	...
	$.extend(F, {
		...
		defaults: {
			...
			tpl: {
				...
				iframe   : '<iframe id="fancybox-frame{rnd}" name="fancybox-frame{rnd}" className="fancybox-iframe" frameborder="0" vspace="0" hspace="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen' + (IE ? ' allowtransparency="true"' : '') + '></iframe>',
				...
				case 'html':
					if (current.selector) {
						...
						if (!content.data(placeholder)) {
							content.data(placeholder, $('<div className="' + placeholder + '"></div>').insertAfter( content ).hide() );
						}
	...
	F.helpers.overlay = {
		...
		create : function(opts) {
			...
			this.overlay = $('<div className="fancybox-overlay"></div>').appendTo( parent && parent.length ? parent : 'body' );
			this.fixed   = false;
	...
	F.helpers.title = {
		...
		beforeShow: function (opts) {
			...
			title = $('<div className="fancybox-title fancybox-title-' + type + '-wrap">' + text + '</div>');
			...
			switch (type) {
				...
				default: // 'float'
					...
					title.wrapInner('<span className="child"></span>');
